...
public class LegacyUrlTest extends AbstractActionRedirectTest {
    ...
    public void testRedirectForJobURL() throws Exception {
        given()
                ...
                .when().get(provider.getJobURL(job) + "redirect").then()
    ...
    public void testRedirectForRunURL() throws Exception {
        given()
                .urlEncodingEnabled(false)
                .redirects().follow(false)
                .when().get(provider.getRunURL(run) + "redirect").then()
                .statusCode(HttpServletResponse.SC_MOVED_TEMPORARILY)
    ...
    public void testRedirectForChangesURL() throws Exception {
        given()
                .urlEncodingEnabled(false)
                .redirects().follow(false)
                .when().get(provider.getRunURL(run) + "redirect?page=changes").then()
                .statusCode(HttpServletResponse.SC_MOVED_TEMPORARILY)
